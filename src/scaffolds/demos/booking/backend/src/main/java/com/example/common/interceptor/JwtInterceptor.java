...
/**
 * JWT认证拦截器
 * 校验Token有效性，解析用户信息，并处理 @RequireAdmin / @RequireProvider 权限
 */
@Component
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        // 服务方权限校验：服务方本人或管理员均可通过
        if (needProvider(handler) && !"provider".equals(role) && !"admin".equals(role)) {
            writeError(response, ResultCode.FORBIDDEN.getCode(), "仅服务方可访问，请先申请入驻");
            return false;
        }
    ...
    /**
     * 判断目标方法或所在类是否标记了@RequireProvider
     */
    private boolean needProvider(Object handler) {
        if (!(handler instanceof HandlerMethod handlerMethod)) {
            return false;
        }
        return handlerMethod.getMethodAnnotation(RequireProvider.class) != null
                || handlerMethod.getBeanType().isAnnotationPresent(RequireProvider.class);
    }
