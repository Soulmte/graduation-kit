<template>
    <a-card title="机构审核">
        <a-alert
            type="info"
            show-icon
            message="审核通过后，负责人账号会自动获得机构角色；封禁则退回普通用户，无法再发布服务与接单。"
        ...
        >
            <template #bodyCell="{ column, record, index }">
                ...
                <template v-else-if="column.key === 'provider'">
                    <div class="pv-cell">
                        ...
                        <div>
                            <div class="pv-name">{{ record.name }}</div>
                            <div class="text-sub">
                                负责人：{{ record.username || "-" }}
                            </div>
                        </div>
                ...
                <template v-else-if="column.key === 'status'">
                    <a-tag :color="PROVIDER_STATUS[record.status]?.color">
                        {{ PROVIDER_STATUS[record.status]?.text }}
                    </a-tag>
                </template>
            ...
            <template #expandedRowRender="{ record }">
                <a-descriptions :column="2" size="small" bordered>
                    ...
                    <a-descriptions-item label="到店地址" :span="2">
                        {{ record.address || "机构未填写" }}
                    </a-descriptions-item>
                    <a-descriptions-item label="申请时间" :span="2">
                        {{ record.createTime }}
                    </a-descriptions-item>
                    <a-descriptions-item label="机构简介" :span="2">
                        {{ record.description || "机构未填写" }}
                    </a-descriptions-item>
                </a-descriptions>
            </template>
        </a-table>
...
const baseColumns = [
    ...
    { title: "机构", key: "provider", width: 220 },
    {
        title: "联系电话",
    ...
    { title: "营业时间", dataIndex: "openTime", key: "openTime", width: 130 },
    { title: "到店地址", dataIndex: "address", key: "address", ellipsis: true },
...
const fetchList = async () => {
    loading.value = true;
    try {
        const res = await pageQueryProvider({
...
const handleAudit = (record, status) => {
    ...
    Modal.confirm({
        ...
        content: isPass
            ? `通过后「${record.name}」即可发布服务并排班，负责人账号将获得机构角色。`
            : `封禁后「${record.name}」无法继续接单，负责人角色会退回普通用户。`,
        okButtonProps: { danger: !isPass },
        onOk: async () => {
            await auditProvider(record.id, status);
