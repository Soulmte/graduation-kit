...
class NoticeService:
    ...
    def add(notice: dict):
        ...
        Database.execute_update(
            'INSERT INTO notice (title, content, create_time) VALUES (%s, %s, NOW())',
            (title, notice.get('content', '')))
    ...
    def page_query(query: dict):
        ...
        records = Database.execute_query(
            f'SELECT * FROM notice {where} ORDER BY {order_col} {direction} LIMIT %s OFFSET %s',
    ...
    def list_all():
        """查询所有公告列表"""
        records = Database.execute_query('SELECT * FROM notice WHERE deleted = 0 ORDER BY create_time DESC')
        for r in records:
            _format_time(r)
        return records

    @staticmethod
    def get_by_id(notice_id: int):
        """根据ID查询公告"""
        notice = Database.execute_one('SELECT * FROM notice WHERE id = %s AND deleted = 0', (notice_id,))
        if not notice:
    ...
    def update(notice: dict):
        ...
        Database.execute_update(
            ...
            (title, notice.get('content', ''), notice_id))

    @staticmethod
    def delete_by_id(notice_id: int):
        """删除公告（逻辑删除）"""
